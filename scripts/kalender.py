...
def kalenderlinks_bauen(site_url):
    ...
    teile = urllib.parse.urlsplit(site_url)
    if teile.scheme and teile.netloc:
        basis = "%s%s" % (teile.netloc, teile.path.rstrip("/"))
    else:
        basis = site_url.rstrip("/")
    webcal = "webcal://%s/%s" % (basis.lstrip("/"), ICS_DATEI)
...
<p class="mt-stack-md font-body-md text-body-md text-on-surface-variant">Unterstützt dein Gerät das Schema webcal, öffnet der erste Knopf direkt deine Kalender-App. Passiert nichts oder meldet die App eine ungültige Adresse, trag diese Adresse dort von Hand als Kalender-Abo ein. Das klappt überall und ist im Google Kalender ohnehin der einzige Weg: <span class="text-on-surface">Weitere Kalender &rarr; Per URL</span>.</p>
